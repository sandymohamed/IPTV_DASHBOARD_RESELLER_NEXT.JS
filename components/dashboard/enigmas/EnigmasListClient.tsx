...
function RowActions({ row }: { row: any }) {
  ...
  const handleEdit = () => {
    router.push(`/dashboard/enigmas/edit/${id}`);
    handleClosePopover();
  };
  ...
  const handleEnableDisable = async () => {
    try {
      setBusy(true);
      const response = await enableDisableEnigma(String(id));
  ...
  const handleLockUnlock = async () => {
    try {
      setBusy(true);
      const response = await lockUnlockEnigma(String(id));
      if (response?.data?.success || response?.success) {
        showToast.success('Success!');
  ...
  const handleKill = async () => {
    try {
      setBusy(true);
      const response = await killEnigmaConnections(String(id));
      if (response?.data?.success || response?.success) {
        showToast.success('Success!');
  ...
  const handleDelete = async () => {
    try {
      setBusy(true);
      const response = await deleteEnigma(String(id));
      if (response?.data?.success || response?.success) {
